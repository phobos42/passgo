...
//adds an item to both the ui tree and underlying data structure(defined in utilTypes)
func addItem(parentNode *tview.TreeNode, reference interface{}) string {
	...
	case **utilTypes.Entry:
		if _, err := reference.(*utilTypes.Item); !err {
			return "Can only create Item inside entry"
		}
		item := reference.(*utilTypes.Item)
...
func indexOfContainer(data []*utilTypes.Container, target **utilTypes.Container) int {
	for k, v := range data {
		if *target == v {
			return k
		}
...
func indexOfEntry(data []*utilTypes.Entry, target **utilTypes.Entry) int {
	for k, v := range data {
		if *target == v {
			return k
		}
...
func indexOfItem(data []*utilTypes.Item, target **utilTypes.Item) int {
	for k, v := range data {
		if *target == v {
			return k
		}
